...
import {BuffaloZclDataType, DataType, StructuredIndicatorType} from './definition/enums.js';
import {BuffaloZclOptions, StructuredSelector, ZclArray} from './definition/tstypes.js';
import {getCluster} from './utils.js';
...
export class BuffaloZcl extends Buffalo {
    ...
    private readGdpFrame(options: BuffaloZclOptions): GPD | GPDChannelRequest | GPDAttributeReport | {raw: Buffer} | Record<string, never> {
        ...
        if (options.payload?.commandID === 0xe0) {
            ...
            const cluster = getCluster(frame.clusterID, frame.manufacturerCode, {});
