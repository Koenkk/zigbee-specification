import type {EUI64} from './definition/tstypes.js';

import {eui64LEBufferToHex} from './utils.js';
...
export class Buffalo {
    ...
    public readIeeeAddr(): EUI64 {
        return eui64LEBufferToHex(this.readBuffer(8));
    }

    public writeBuffer(values: Buffer | number[], length: number): void {
        if (values.length !== length) {
            throw new Error(`Length of values: '${values.toString('hex')}' is not consitent with expected length '${length}'`);
        }
