import {Zcl} from '../../src/index.js';
import {uint16To8Array, uint32To8Array, uint56To8Array} from '../utils.js';
...
const GLOBAL_RSP_FRAME_STRING = `{"header":{"frameControl":{"reservedBits":0,"frameType":0,"direction":1,"disableDefaultResponse":false,"manufacturerSpecific":false},"transactionSequenceNumber":78,"commandIdentifier":1},"payload":[{"attrId":256,"status":0,"dataType":48,"attrData":127}],"command":{"ID":1,"name":"readRsp","parameters":[{"name":"attrId","type":33},{"name":"status","type":32},{"name":"dataType","type":32,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]},{"name":"attrData","type":1000,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]}]}}`;
...
const GLOBAL_CONDITION_FRAME_STRING = `{"header":{"frameControl":{"reservedBits":0,"frameType":0,"direction":1,"disableDefaultResponse":false,"manufacturerSpecific":false},"transactionSequenceNumber":78,"commandIdentifier":6},"payload":[{"direction":1,"attrId":256,"timeout":10000}],"command":{"ID":6,"name":"configReport","parameters":[{"name":"direction","type":32},{"name":"attrId","type":33},{"name":"dataType","type":32,"conditions":[{"type":${Zcl.ParameterCondition.DIRECTION_EQUAL},"value":${Zcl.Direction.CLIENT_TO_SERVER}}]},{"name":"minRepIntval","type":33,"conditions":[{"type":${Zcl.ParameterCondition.DIRECTION_EQUAL},"value":${Zcl.Direction.CLIENT_TO_SERVER}}]},{"name":"maxRepIntval","type":33,"conditions":[{"type":${Zcl.ParameterCondition.DIRECTION_EQUAL},"value":${Zcl.Direction.CLIENT_TO_SERVER}}]},{"name":"repChange","type":1000,"conditions":[{"type":${Zcl.ParameterCondition.DIRECTION_EQUAL},"value":${Zcl.Direction.CLIENT_TO_SERVER}},{"type":${Zcl.ParameterCondition.DATA_TYPE_CLASS_EQUAL},"value":${Zcl.DataTypeClass.ANALOG}}]},{"name":"timeout","type":33,"conditions":[{"type":${Zcl.ParameterCondition.DIRECTION_EQUAL},"value":${Zcl.Direction.SERVER_TO_CLIENT}}]}],"response":7}}`;
...
const SPECIFIC_CONDITION_FRAME_STRING = `{"header":{"frameControl":{"reservedBits":0,"frameType":1,"direction":1,"disableDefaultResponse":false,"manufacturerSpecific":false},"transactionSequenceNumber":45,"commandIdentifier":2},"payload":{"status":149},"command":{"ID":2,"parameters":[{"name":"status","type":32},{"name":"manufacturerCode","type":33,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]},{"name":"imageType","type":33,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]},{"name":"fileVersion","type":35,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]},{"name":"imageSize","type":35,"conditions":[{"type":${Zcl.ParameterCondition.STATUS_EQUAL},"value":${Zcl.Status.SUCCESS}}]}],"name":"queryNextImageResponse"}}`;
...
describe('ZCL Frame', () => {
    ...
    describe('Header', () => {
        it.each([
            ...
            // write
            {
                ...
                const buffalo = new Zcl.Buffalo(buffer);
    ...
    it('Writes & Reads repetitive strategy', () => {
        ...
        const frame = Zcl.Frame.create(
            ...
            expected /*payload*/,
            {} /*custom clusters*/,
            // header.frameControl.reservedBits, // coverage
        );
    ...
    it('Reads list type parameter', () => {
        const expected = {groupcount: 2, grouplist: [10, 20]};
        const header = new Zcl.Header(
            {
                reservedBits: 0,
                direction: 0,
        ...
        const readHeader = Zcl.Header.fromBuffer(buffer)!;
        const frame = Zcl.Frame.fromBuffer(Zcl.Clusters.genGroups.ID, readHeader, buffer, {});
