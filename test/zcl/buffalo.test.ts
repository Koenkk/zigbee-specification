...
describe('ZCL Buffalo', () => {
    ...
    it('[workaround] Reads char str as Mi struct with trailing byte for Xiaomi attridId=65281', () => {
        const expectedValue = {
            '3': 29,
            '5': 11710,
            '8': 4655,
    ...
    it.each([
        ...
        const buffalo = new Zcl.Buffalo(Buffer.alloc(50, 0xff));
    ...
    it('Throws when write Use Data Type is missing dataType option and value isnt buffer or number array', () => {
        expect(() => {
            const payload = 1;
            const buffalo = new Zcl.Buffalo(Buffer.alloc(2));
    ...
    it.each([
        ...
        expect(() => {
            const buffalo = new Zcl.Buffalo(Buffer.alloc(1));
            buffalo.read(type, {});
        }).toThrow();
    });
    ...
    describe('GPD Frame', () => {
        it('Reads unhandled command as object[raw] if buffer still has bytes to read', () => {
            ...
            const buffalo = new Zcl.Buffalo(Buffer.from(value));
        ...
        it('Reads unhandled command as empty object if buffer finished reading', () => {
            const value = [0xff, 0x00];
            const buffalo = new Zcl.Buffalo(Buffer.from(value), value.length /* pos at end*/);
        ...
        it('Writes commissioning all options', () => {
            ...
            const buffalo = new Zcl.Buffalo(Buffer.alloc(28));
        ...
        it('Writes custom reply', () => {
            ...
            const buffalo = new Zcl.Buffalo(Buffer.alloc(7));
    ...
    it.each([Zcl.DataType.UINT40, Zcl.DataType.DATA40, Zcl.DataType.BITMAP40])('Writes & reads uint40, data40, bitmap40', (type) => {
        const value = 25853953281;
        const expectedWritten = [1, 5, 4, 5, 6];
        const expectedPosition = 5;
        const buffer = Buffer.alloc(10);
        const buffalo = new Zcl.Buffalo(buffer);
        buffalo.write(type, value, {});
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
        expect(buffalo.getWritten()).toStrictEqual(Buffer.from(expectedWritten));
        ...
        expect(buffalo.read(type, {})).toStrictEqual(value);
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
    });

    it.each([Zcl.DataType.UINT48, Zcl.DataType.DATA48, Zcl.DataType.BITMAP48])('Writes & reads uint48, data48, bitmap48', (type) => {
        const value = 7722435347713;
        const expectedWritten = [1, 5, 4, 5, 6, 7];
        const expectedPosition = 6;
        const buffer = Buffer.alloc(10);
        const buffalo = new Zcl.Buffalo(buffer);
        buffalo.write(type, value, {});
    ...
    it.each([Zcl.DataType.UINT56, Zcl.DataType.DATA56, Zcl.DataType.BITMAP56])('Writes & reads uint56, data56, bitmap56', (type) => {
        const value = 1978047272322305n;
        const expectedWritten = [1, 5, 4, 5, 6, 7, 7];
    ...
    it.each([Zcl.DataType.UINT64, Zcl.DataType.DATA64, Zcl.DataType.BITMAP64])('Writes & reads uint64, data64, bitmap64', (type) => {
        const value = 0x0907070605040501n;
        const expectedWritten = [1, 5, 4, 5, 6, 7, 7, 9];
    ...
    it('Writes & reads int40', () => {
        const value = 549755813886;
        const expectedWritten = [254, 255, 255, 255, 127];
        const expectedPosition = 5;
        const buffer = Buffer.alloc(10);
        const buffalo = new Zcl.Buffalo(buffer);
        buffalo.write(Zcl.DataType.INT40, value, {});
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
        expect(buffalo.getWritten()).toStrictEqual(Buffer.from(expectedWritten));
        ...
        expect(buffalo.read(Zcl.DataType.INT40, {})).toStrictEqual(value);
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
    });

    it('Writes & reads int56', () => {
        const value = 1978047272322305n;
        const expectedWritten = [1, 5, 4, 5, 6, 7, 7];
        const expectedPosition = 7;
        const buffer = Buffer.alloc(10);
        const buffalo = new Zcl.Buffalo(buffer);
        buffalo.write(Zcl.DataType.INT56, value, {});
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
        expect(buffalo.getWritten()).toStrictEqual(Buffer.from(expectedWritten));
        ...
        expect(buffalo.read(Zcl.DataType.INT56, {})).toStrictEqual(value);
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
    });

    it('Writes & reads int64', () => {
        const value = 650496393613673729n;
        const expectedWritten = [1, 5, 4, 5, 6, 7, 7, 9];
        const expectedPosition = 8;
        const buffer = Buffer.alloc(10);
        const buffalo = new Zcl.Buffalo(buffer);
        buffalo.write(Zcl.DataType.INT64, value, {});
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
        expect(buffalo.getWritten()).toStrictEqual(Buffer.from(expectedWritten));
        ...
        expect(buffalo.read(Zcl.DataType.INT64, {})).toStrictEqual(value);
        expect(buffalo.getPosition()).toStrictEqual(expectedPosition);
    });
});
