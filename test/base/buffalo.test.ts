import {ZSpec} from '../../src/index.js';
import {IEEE_ADDR1, IEEE_ADDR2} from '../utils.js';

describe('Buffalo', () => {
    it('is more', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0, 1, 2]), 1);
    ...
    it('gets written', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(3), 1);
    ...
    it('UINT8 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x03, 0x00, 0x00]), 1);
    ...
    it('INT8 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0xf0, 0x00, 0x00]), 1);
    ...
    it('INT16 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0xff, 0xff, 0x00]), 1);
    ...
    it('UINT16 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x03, 0xff, 0x00]), 1);
    ...
    it('INT32 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(6), 2);
    ...
    it('INT32 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x01, 0x03, 0xff, 0xff]));
    ...
    it('INT40 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(7), 2);
    ...
    it('INT40 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([1, 2, 3, 4, 5]));
    ...
    it('INT48 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(8), 2);
    ...
    it('INT48 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([1, 2, 3, 4, 5, 6]));
    ...
    it('UINT56 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(9), 2);
    ...
    it('UINT56 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([254, 255, 255, 255, 255, 255, 255]));
    ...
    it.each([
        ...
        const buffalo = new ZSpec.Buffalo(Buffer.from(values));
    ...
    it('UINT64 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(10), 2);
    ...
    it('UINT64 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([253, 255, 255, 255, 255, 255, 255, 255]));
    ...
    it('INT64 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([254, 255, 255, 255, 255, 255, 255, 127]));
    ...
    it('IEEEADDR write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(8));
        buffalo.writeIeeeAddr(IEEE_ADDR1.string);
        expect(buffalo.getPosition()).toEqual(8);
        expect(buffalo.getBuffer()).toStrictEqual(Buffer.from(IEEE_ADDR1.hex));
    });

    it('IEEEADDR read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from(IEEE_ADDR2.hex));
        const value = buffalo.readIeeeAddr();
        expect(buffalo.getPosition()).toEqual(8);
        expect(value).toStrictEqual(IEEE_ADDR2.string);
    });

    it('BUFFER write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(5), 1);
    ...
    it('BUFFER write invalid length', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(5), 1);
        const payload = Buffer.from([0x00, 0x01, 0x02]);
        expect(() => {
            buffalo.writeBuffer(payload, payload.length - 1);
        }).toThrow(`Length of values: '${payload.toString('hex')}' is not consitent with expected length '${payload.length - 1}'`);
    });

    it('BUFFER read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09]), 2);
    ...
    it('LIST_UINT8 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(4), 1);
    ...
    it('LIST_UINT8 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x00, 0x04, 0x08]), 2);
    ...
    it('LIST_UINT16 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x00, 0x04, 0x00, 0x08]), 1);
    ...
    it('LIST_UINT24 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(8), 1);
    ...
    it('LIST_UINT24 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0xa0, 0x86, 0x01, 0x1e, 0xae, 0x01, 0x00]), 1);
    ...
    it('LIST_UINT32 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0xff, 0xff, 0xff, 0xff, 0x0a, 0x00, 0x00, 0x00]));
    ...
    it('UINT24 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x01, 0xa0, 0x86, 0x01, 0x02]), 1);
    ...
    it('INT24 write +', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(3));
    ...
    it('INT24 read +', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x7f, 0xff, 0xff, 0x7f]), 1);
    ...
    it('INT24 read -', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x00, 0xff]));
        const value = buffalo.readInt24();
        expect(buffalo.getPosition()).toStrictEqual(3);
    ...
    it('INT32 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(4));
    ...
    it('INT32 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x08, 0xff, 0x03, 0xff]));
        const value = buffalo.readInt32();
        expect(buffalo.getPosition()).toStrictEqual(4);
    ...
    it('INT48 write', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.alloc(6));
    ...
    it('INT48 read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0xff, 0x03, 0xff, 0x03, 0xff]));
        const value = buffalo.readInt48();
        expect(buffalo.getPosition()).toStrictEqual(6);
    ...
    it('FLOATLE read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x00, 0x40, 0xc8, 0x43]));
    ...
    it('DOUBLELE read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x33, 0x33, 0x33, 0x33, 0x31, 0xb7, 0x28, 0x41]), 0);
    ...
    it('UTF8 STRING read', () => {
        const buffalo = new ZSpec.Buffalo(Buffer.from([0x61, 0x62, 0x63, 0x64]), 0);
